...
- [Crosswire](https://crosswire-api.com) — Pre-trade resolution check for cross-venue arbitrage: before you fire both legs of a Polymarket/Kalshi hedge, returns SAFE / CAUTION / BLOCK on whether the two contracts actually settle the same way (resolution source, settlement instant, void rules) and names the exact divergence. Free demo, no key; $0.02/call via x402 on Base; advisory only, never executes.
