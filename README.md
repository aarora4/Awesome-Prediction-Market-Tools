...
- [PredRadar](https://predradar.com/) - Cross-platform aggregator for Polymarket, Kalshi, and Betfair with live prices, an arbitrage scanner, a JSON API, and an MCP server for AI agents.
...
- [PredCopy](https://predcopy.com/) - Automated copy trading for Polymarket, pairing whale tracking and a composite trader score (P&L, win rate, recent activity) with an execution engine that mirrors a followed wallet's trades under configurable bet sizing, max exposure, and stale position exit rules. Includes Telegram alerts and a remote MCP server so AI agents can rank traders and start or stop copying.
- [predtools](https://predtools.com/) - Source-available trading bots for Polymarket and Kalshi with a strategy library, sold as one-time purchases rather than a subscription.
