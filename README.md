...
- [Live Tennis API](https://livetennisapi.com) — Live-tennis match-state data feed (point-by-point scores, who's serving, a three-valued break-point flag, and retirement/walkover/completed status) that serves as the real-time ground truth for tennis event markets on Polymarket and Kalshi. Free tier, no card, 30 req/min; REST plus push over WebSocket. Ships an MIT toolkit, [polymarket-tennis](https://github.com/livetennisapi/polymarket-tennis), for Gamma tennis-market discovery and match↔market matching, plus an MCP server (livetennisapi-mcp) on the official MCP Registry.
