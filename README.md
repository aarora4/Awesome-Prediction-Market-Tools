...
- [ImpliedData](https://implieddata.com) — Cross-platform prediction-market data: the same event matched across Polymarket, Manifold & Myriad (polarity-aligned, so YES means the same thing), plus the Polymarket on-chain tick history (404M fills, 2023–present). YES-normalized candles, event links & clusters, CSV/Parquet/API.
