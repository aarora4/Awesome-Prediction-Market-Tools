...
- [outcometick](https://outcometick.com) — Tick-level historical data for Polymarket and Predict.fun crypto Up/Down markets, including the full-precision Chainlink settlement feeds and the per-market strike, so wins and losses can be recomputed from source rather than trusted from a label. Self-serve HTTP API plus a sandboxed backtest runner with npm and PyPI SDKs.
