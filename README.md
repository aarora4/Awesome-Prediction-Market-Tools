...
- [OraclePoly](https://github.com/Socialpranker/oraclepoly) — Self-trading execution bot for Polymarket: ~20 detectors → ML calibration (Beta + LightGBM) → EV/Kelly risk sizing → CLOB execution, gated through backtest → shadow → virtual → live stages before any money moves. Open-sourced with an honest retrospective on what didn't work (execution costs ate the edge) and what's still open (cross-platform arb with Kalshi). MIT.
