...
- [Whalefax](https://whalefax.com/) - Free Polymarket whale leaderboard and full wallet history reconstruction that recovers losing positions which were never redeemed on chain, since redeeming a loser costs gas and pays nothing, meaning most published P&L figures count the wins and quietly drop the losses. Also produces per-position P&L and tax records for Kalshi, Polymarket and Robinhood event contracts with FIFO lot matching. No account, no wallet connection, runs entirely client side.
